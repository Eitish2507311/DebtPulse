// Enum option lists mirrored from the backend (used to populate selects).
export const ENUMS: Record<string, string[]> = {
  ...
  CaseStatus: ['FILED', 'PENDING', 'HEARING_SCHEDULED', 'DECREED', 'WITHDRAWN', 'SETTLED'],
...
export const bucketClass = (b?: string): string =>
  ['NPA', 'WRITEOFF', 'X180'].includes(b || '') ? 's-red'
    : ['X120', 'X90'].includes(b || '') ? 's-amber' : 's-blue';
