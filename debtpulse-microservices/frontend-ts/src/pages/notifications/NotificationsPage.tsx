...
import { PageHeader, ErrorNote, EmptyState, Loading, Pill } from '../../components/ui';
...
export default function NotificationsPage() {
  ...
  const fetcher = useCallback((p: Record<string, unknown>) => notificationApi.list(p), []);
  const { page, loading, error, setPage, reload } = usePaged<Notification>(fetcher);

  const act = async (fn: () => Promise<unknown>, msg: string) => {
    try { await fn(); toast.success(msg); reload(); } catch { toast.error('Action failed'); }
  };
