...
import { titleCase, num } from '../../utils/format';
import { useAuth } from '../../auth/AuthContext';
import type { AllocationRule, Column } from '../../types';

export default function AllocationsPage() {
  const toast = useToast();
  const { role } = useAuth();
  const canWrite = role === 'ADMIN' || role === 'PORTFOLIO_MANAGER' || role === 'COLLECTIONS_AGENT';
  const fetcher = useCallback((p: Record<string, unknown>) => allocationApi.list(p), []);
  const { page, loading, error, setPage, reload } = usePaged<AllocationRule>(fetcher);
  ...
  const columns: Column<AllocationRule>[] = [
  ...
  const isEdit = !!(editing && (editing as AllocationRule).ruleId);
  ...
  const editInitial = isEdit ? { ...(editing as AllocationRule), active: (editing as AllocationRule).active ?? true } : empty;
  ...
  return (
    <>
      ...
      <FormModal show={!!editing} title={isEdit ? 'Edit Allocation Rule' : 'New Allocation Rule'}
        ...
        onSubmit={(v) => {
          const numOrNull = (x: unknown) => (x === '' || x == null ? null : Number(x));
          const body = { ...v,
          ...
          return isEdit ? allocationApi.update((editing as AllocationRule).ruleId, body) : allocationApi.create(body);
        }}>
        {(v, set, errs) => (
          <Row>
