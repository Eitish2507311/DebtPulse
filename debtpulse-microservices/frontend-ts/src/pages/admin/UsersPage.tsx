...
import type { UserDto, Column } from '../../types';

export default function UsersPage() {
  const toast = useToast();
  const fetcher = useCallback((p: Record<string, unknown>) => userApi.list(p), []);
  const { page, loading, error, setPage, reload } = usePaged<UserDto>(fetcher);
  const [create, setCreate] = useState(false);
  const [edit, setEdit] = useState<UserDto | null>(null);
  const [remove, setRemove] = useState<UserDto | null>(null);
  const [busy, setBusy] = useState(false);

  const setStatus = async (u: UserDto, status: string) => {
  ...
  return (
    <>
      ...
      <DataTable<UserDto> columns={columns} page={page} loading={loading} onPageChange={setPage} emptyIcon="people" emptyTitle="No users" />
