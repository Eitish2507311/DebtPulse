...
import type { LegalCase, CourtHearing } from '../../types';
...
export default function LegalCaseDetailPage() {
  ...
  const { data: c, loading, error, reload } = useAsync<LegalCase>(() => legalApi.getCase(id), [id]);
  const hearings = useAsync<CourtHearing[]>(() => legalApi.listHearings(id), [id]);
