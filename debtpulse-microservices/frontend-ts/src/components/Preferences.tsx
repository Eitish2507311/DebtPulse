...
export function PreferencesProvider({ children }: { children: ReactNode }) {
  ...
  const stepFont = useCallback((dir: number) => setFontScale((f) => {
  ...
  const value = useMemo<Preferences>(
    () => ({ theme, toggleTheme, fontScale, stepFont, resetFont, canGrow: fontScale < FONT_STEPS[FONT_STEPS.length - 1], canShrink: fontScale > FONT_STEPS[0] }),
