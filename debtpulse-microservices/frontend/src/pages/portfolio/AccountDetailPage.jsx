...
import { accountApi, collateralApi, borrowerContactApi } from '../../api/services.js';
...
import { ENUMS, bucketClass } from '../../utils/enums.js';
import { inr, num, date, titleCase } from '../../utils/format.js';
...
export default function AccountDetailPage() {
  ...
  const { role } = useAuth();
  const canEdit = [ROLES.ADMIN, ROLES.COLLECTIONS_AGENT].includes(role);
  // Field officers get read-only portfolio access (they look up asset ids for verification but make
  // no changes here); collateral is registered at onboarding or by agents/admins.
  const canCollateral = [ROLES.ADMIN, ROLES.COLLECTIONS_AGENT].includes(role);

  const { data: acc, loading, error, reload } = useAsync(() => accountApi.get(id), [id]);
  ...
  const info = [
    ...
    ['Principal', inr(acc.principalAmount)], ['Total Overdue', inr(acc.totalOverdue)],
    ['DPD', num(acc.dpd)],
  ];
    ...
    <>
      ...
      <Tabs defaultActiveKey="overview" className="mb-3">
        ...
        <Tab eventKey="contacts" title="Borrower Contacts">
          <BorrowerContactsTab accountId={id} canWrite={[ROLES.ADMIN, ROLES.COLLECTIONS_AGENT].includes(role)} />
        </Tab>
      </Tabs>
...
function BorrowerContactsTab({ accountId, canWrite }) {
  const { data, loading, reload } = useAsync(() => borrowerContactApi.byAccount(accountId), [accountId]);
  const [show, setShow] = useState(false);
  if (loading) return <Loading />;
  return (
    <Card><Card.Header className="d-flex justify-content-between align-items-center">
